 ...
 * 终端输出缓冲。
 *
 * 后端在 `createLocal` 返回时就已经开始产出数据（Windows ConPTY 会立刻发
 * `\x1b[6n` 查询光标位置并等待应答），而 TerminalView 要等 React 挂载完成才
 * 能订阅。若此时没有订阅者，这些字节会被直接丢弃：xterm 收不到 DSR 查询就
 * 不会回应答，ConPTY 一直等下去，表现就是「本地终端连不上、只有光标」。
 *
 * 所以订阅在 bootstrap 阶段就建立：已有 TerminalView 的会话直接转发，
 * 还没有的先进缓冲，TerminalView 挂载时一次性回放。
 */
const terminalDataHandlers = new Map<string, (data: Uint8Array) => void>()
const terminalDataPending = new Map<string, Uint8Array[]>()
/** 单个会话的待回放缓冲上限（与后端输出缓冲同量级），防止无人认领时无限增长 */
const PENDING_LIMIT = 256 * 1024

function bufferTerminalData(sessionId: string, data: Uint8Array): void {
  const pending = terminalDataPending.get(sessionId) ?? []
  pending.push(data)
  let total = 0
  for (const chunk of pending) total += chunk.byteLength
  while (pending.length > 1 && total > PENDING_LIMIT) {
    total -= pending.shift()!.byteLength
  }
  terminalDataPending.set(sessionId, pending)
}

/**
 * 订阅某个会话的终端输出，返回取消订阅函数。
 * 注册与回放同步完成，所以既不会丢字节也不会重复。
 */
export function attachTerminalData(
  sessionId: string,
  handler: (data: Uint8Array) => void
): () => void {
  terminalDataHandlers.set(sessionId, handler)
  const pending = terminalDataPending.get(sessionId)
  if (pending) {
    terminalDataPending.delete(sessionId)
    for (const chunk of pending) handler(chunk)
  }
  return () => {
    if (terminalDataHandlers.get(sessionId) === handler) terminalDataHandlers.delete(sessionId)
  }
}

/** 会话结束后释放缓冲，避免残留字节长期占用内存 */
export function dropTerminalData(sessionId: string): void {
  terminalDataHandlers.delete(sessionId)
  terminalDataPending.delete(sessionId)
}
...
/**
 * 绑定主进程推送的事件（终端输出 / 退出 / 连接阶段 / AI 流式 / 监控）。
 *
 * 必须在 `installApi()` 之后调用：ES 模块的 import 会被提升，若放在 store 工厂里
 * 执行，`window.api` 此时还是 undefined，整块监听会静默不注册——表现就是终端
 * 收不到任何输出（ConPTY 启动时的 DSR 查询无人应答，shell 永久卡住）。
 */
export function bindAppListeners(): void {
  if (listenersBound || typeof window === 'undefined' || !window.api) return
  listenersBound = true
  // 终端输出统一在这里接：TerminalView 挂载前到达的字节先缓冲，避免 ConPTY
  // 启动时的 DSR 查询（\x1b[6n）丢失导致 shell 永久阻塞
  window.api.terminal.onData(({ sessionId, data }) => {
    const handler = terminalDataHandlers.get(sessionId)
    if (handler) handler(data)
    else bufferTerminalData(sessionId, data)
  })
  // 会话输出退出等事件 -> 更新状态
  ...
  window.api.terminal.onClosed(({ sessionId }) => {
    ...
    useAppStore.setState((s) => applyTabClose(s, sessionId))
  })
  window.api.ai.onChatEvent(({ requestId, event }) => {
    useAppStore.getState().handleAiEvent(requestId, event)
  })
  window.api.ai.onConfirmRequest((req) => {
    // 每个会话的助手实例独立弹卡（同一实例内已由主进程串行化）
    useAppStore.setState((s) => ({ pendingConfirms: { ...s.pendingConfirms, [req.id]: req } }))
  })
