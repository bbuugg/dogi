 ...
 *
 * Monaco Editor 默认从 CDN 加载资源；把 min 产物拷贝到 public 目录后，
 * 配合 MonacoEditor.tsx 里的 loader.config({ paths }) 即可完全离线可用。
 *
...
console.log(`[copy-monaco] 已复制 monaco-editor/min@${version} → src/renderer/public/monaco-editor`)
