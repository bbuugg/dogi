...
export function TitleBar() {
  ...
  return (
    <header className="app-drag flex h-9 shrink-0 items-stretch bg-sidebar">
      {isMac ? (
        <div className="w-[76px] shrink-0" />
      ) : (
        <div className="app-no-drag flex w-36 shrink-0 items-center gap-2 pl-3">
          <img src={appIcon} alt="Dogi" className="size-5" draggable={false} />
          <span className="text-xs font-semibold">Dogi</span>
        </div>
      )}
