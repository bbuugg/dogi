...
export function ApiPage({ requestId }: { requestId: string }) {
  ...
  /**
   * 进行中请求的取消句柄：sendIdRef 记着本次 api:send 的 sendId（主进程按它
   * 找到 AbortController），cancelledRef 标记「是用户主动取消的」——
   * 取消与超时在主进程侧都表现为 status=0 + AbortError，只有渲染端能区分。
   */
  ...
  /**
   * 消费「历史记录载入」的种子：侧边栏历史分区点「载入」时，store 先存一份条目内容
   * 再打开（或聚焦）新建请求草稿，本 effect 把种子写进草稿后立即清空。
   * 只作用于 HTTP 草稿（isDraft）：已保存请求的标签也挂着本组件，绝不能被种子冲掉草稿；
   * 声明在「切换请求」effect 之后 —— 挂载时两者都跑，种子要在其后覆盖生效。
   */
  const apiDraftSeed = useAppStore((s) => s.apiDraftSeed)
  const consumeApiDraftSeed = useAppStore((s) => s.consumeApiDraftSeed)
  useEffect(() => {
    if (!isDraft || !apiDraftSeed) return
    consumeApiDraftSeed()
    const seedHeaders = normalizeHeaders(apiDraftSeed.headers)
    setMethod(apiDraftSeed.method || 'GET')
    setUrl(apiDraftSeed.url || '')
    ...
    setReqTab('headers')
    markDirty()
  }, [apiDraftSeed, consumeApiDraftSeed, isDraft])
  ...
  const send = async (): Promise<void> => {
    ...
    setError(null)
    setResponse(null)
    const sendId = String(Date.now()) + Math.random().toString(36).slice(2, 8)
    ...
    try {
      ...
      if (cancelledRef.current) {
        ...
        // 失败也记历史（status=0），便于回看「当时发的是什么」；手动取消不记
  ...
  /** 取消进行中的请求（发送按钮在请求期间就是「取消」） */
  const cancelSend = (): void => {
    const id = sendIdRef.current
    if (!id) return
    cancelledRef.current = true
    void window.api.apiClient.abort(id)
  }
  ...
  return (
    <div ref={rootRef} className="flex h-full flex-col bg-background" onKeyDown={onKeyDown}>
      ...
      >
        ...
        />
        {/* 请求进行中时同一颗按钮切换为「取消」：点它中断请求（Ctrl+Enter 同效） */}
        {sending ? (
          <Button
            danger
            icon={<X className="size-4" />}
            onClick={cancelSend}
            title="取消请求（Ctrl+Enter）"
          >
            取消
          </Button>
        ) : (
                  ...
                  <div className="select-text space-y-0.5">
                    {respHeaders.map(([k, v]) => (
                      <div key={k} className="flex gap-3 border-b border-border/40 py-1">
                        <span className="w-56 shrink-0 break-all font-mono text-xs text-muted-foreground">
                          {k}
                        </span>
