...
const MonacoEditor: FC<MonacoEditorProps> = ({
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden bg-background">
      {/* Toolbar：showHeader=false 时整条收起（只留编辑区），用 hidden 而非不渲染 —— 
          工具栏里有依赖 mounted 的按钮，卸载重建会多一次挂载开销 */}
      <div
        className={cn(
        ...
        <div className="flex min-w-0 items-center gap-1.5">
          {showLanguageSelector ? (
            <Select
              ...
              // bare-select：与 Agent 输入框里的模型选择一致 —— 按下（展开）也不冒边框
              // （antd 会给聚焦的 Select 补一圈 focus outline，见 index.css）
              className="bare-select text-xs! font-medium!"
            />
          ) : (
