          ...
          {root ? (
            <>
              <div className="flex items-baseline gap-1.5">
                ...
                <span className="text-xs font-normal text-muted-foreground">{root.mount}</span>
              </div>
              <div className="mt-1">
...
export function MonitorBadge({ sessionId }: { sessionId: string | null }) {
  ...
  return (
    <Popover
      ...
      trigger="click"
      // 指标条已移到状态栏右侧，气泡也跟着贴右边缘展开（否则 300px 宽会被窗口挤回左边）
      placement="topRight"
      arrow={false}
      destroyOnHidden
    ...
    >
      <button
        ...
        aria-label="服务器指标详情"
        title="服务器指标"
        className="flex h-6 items-center gap-3 rounded px-1.5 text-xs whitespace-nowrap text-muted-foreground transition-colors hover:bg-sidebar-accent hover:text-foreground"
      >
        <span
          className={cn(
