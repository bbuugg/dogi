  ...
  return (
    <div ref={rootRef} className="flex h-full flex-col bg-background" onKeyDown={onKeyDown}>
      ...
      <div className="flex shrink-0 items-center gap-2 px-3 py-1.5">
        ...
        {isOpen && protocol && (
          <span className="shrink-0 text-[11px] text-muted-foreground" title="服务端协商出的子协议">
            子协议 {protocol}
          </span>
        )}
        <Button
      ...
      <div ref={connRowRef} className="flex shrink-0 items-center gap-2 px-3 py-2">
        <span className="shrink-0 rounded bg-secondary px-1.5 py-0.5 font-mono text-[11px] font-semibold text-sky-500">
          WS
        </span>
        <Input
          value={url}
      ...
      {connError && (
        <div className="flex shrink-0 items-center gap-1.5 px-3 pb-1 text-[11px] text-destructive">
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
        <div className="flex shrink-0 items-center gap-3 px-3 py-1.5 text-xs">
          ...
          <Button
            type="text"
            size="small"
            className="ml-auto shrink-0 gap-1.5 px-2 text-[11px] text-muted-foreground"
    ...
    >
      ...
      {isSystem ? (
        <>
          ...
          <span
            className={cn(
              'min-w-0 flex-1 whitespace-pre-wrap break-all text-[11px] leading-4',
              entry.level === 'error' ? 'text-destructive' : 'text-muted-foreground'
            )}
          >
            {entry.data}
        ...
        <>
          ...
          <pre className="max-h-48 min-w-0 flex-1 overflow-auto whitespace-pre-wrap break-all font-mono text-[11px] leading-4">
            {text}
          </pre>
        </>
      )}
